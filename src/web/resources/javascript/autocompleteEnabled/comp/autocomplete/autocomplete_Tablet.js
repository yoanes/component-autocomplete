...
var AutoCompleteTabletPrototype = new Class({
	...
	initialize: function(toObserve, toPopulate, toURL, preDataAdaptor, postDataAdaptor, minChar, limit, ulCSS, proxyUrl) {
		...
		this.closeLink.style.cssFloat = 'right';
		this.closeLink.style.marginLeft = '2px';
			...
			if(qval.length >= this.minChar){
				if(qval != this.lastQuery && this.URL.length > 0) {
					...
					if(this.URL instanceof Function) {
						...
						}).send('url=' + encodeURIComponent(this.URL + finalQuery) + '&limit=' + this.limit);
					}
					this.lastQuery = qval;
				}
			}
